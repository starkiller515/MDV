...
window.addEventListener("DOMContentLoaded", function () {
    ...
    var titleList = ["--Choose a Title--", "Batman", "Nightwing", "Dark Knight", "Batman and Robin", "Catwoman", "Detective Comics"];
...
// turn form on and off
    ...
    function retrieveData(){
        ...
        if (localStorage.length === 0){
            alert("No books found in local storage, loaded default data.");
            autoPop();
        }
        ...
        for(var i=0, j=localStorage.length; i < j; i++){
            ...
            appImage(bookStats.title[1], createSubList);
    ...
    //Get image for each item in local storage
    function appImage(picName, createSubList){
        var imgLine = document.createElement("li");
        createSubList.appendChild(imgLine);
        var newPic = document.createElement("img");
        var picSource = newPic.setAttribute("src", "images/" + picName + ".jpg");
        imgLine.appendChild(newPic);
    }

    //auto populate dummy json data if there is nothing there to display when Display Data is clicked
    function autoPop(){
        for(var n in json){
            var id = Math.floor(Math.random()*120211);
